...
class Animal(ABC):
    ...
    async def move(self, direct):
        ...
        if str(self.simulation.environment.groundMatrix[self.limiter(self.cellList[0].y + delta[0], "height")][self.limiter(self.cellList[0].x + delta[1], "width")]) in self.waylessGround:
            return None
        elif self.simulation.animalMatrix[self.limiter(self.cellList[0].y + delta[0], "height")][self.limiter(self.cellList[0].x + delta[1], "width")] != None:
            return None
