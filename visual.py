...
class interface_op:
    ...
    def validation(self,data_array, parent, mode = False):
        if mode:
            for i in range(len(data_array)):
                if data_array[i].isdigit() and int(data_array[i]) >= 0:
                    if i == 2 and (int(data_array[i-1])+int(data_array[i])) != 100:
                        messagebox.showerror("Невірно введені дані", "Значення мають у сумі дорівнювати 100. Введіть значення ще раз", parent=parent)
                        return False
                    if (i == 5 or i == 8)  and (int(data_array[i-1])+int(data_array[i - 2])+int(data_array[i])) != 100:
                        messagebox.showerror("Невірно введені дані", "Значення мають у сумі дорівнювати 100. Введіть значення ще раз", parent=parent)
                        return False
                    if i == 9  and (int(data_array[i]) == 0  or  int(data_array[i]) > 10000):
                        messagebox.showerror("Невірно введені дані", "Кількість днів не має перевищувати 100 або дорівнювати 0. Введіть значення ще раз.", parent=parent)
                        return False
                    if  i == 10 and int(data_array[i]) == 0:
                        messagebox.showerror("Невірно введені дані", "Кількість циклів не має дорівнювати 0. Введіть значення ще раз.", parent=parent)
                        return False
            ...
            for i in range(len(data_array)):
                if data_array[i].isdigit() == False or int(data_array[i]) < 0:
                    ...
                    return False
            if int(data_array[2]) + int(data_array[3]) +int(data_array[4]) > 100 or int(data_array[2]) + int(data_array[3]) +int(data_array[4]) == 0:
                messagebox.showerror("Невірно введені дані", "Частка води, землі та трави у сумі не має перевщувати 100 або дорівнювати 0. Введіть значення ще раз.", parent= parent)
                return False
            if (int(data_array[0]) > 10 and int(data_array[0]) <= 130 )  and (int(data_array[1]) > 10 and int(data_array[1]) <= 235):
    ...
    def centerWindow(self, window, width, height):
        w = window.parent.winfo_screenwidth()//2-(width//2)
        h = window.parent.winfo_screenheight()//2-(height//2)
        window.parent.geometry('{}x{}+{}+{}'.format(width,height, w, h))


    def fill_table(self, files, list_box):
        for i in files:
            list_box.insert(0,i)
    ...
    def add_go_back(window, parent):
        Button(window.frame, text = "Назад", font = "Georgia 10 bold" , bg = "#71ed74", foreground = "white", width = 5, command = lambda: interface_op.close(parent)).pack(side = LEFT)

    
    @staticmethod 
    def init_window(window,parent,title, w, h):
        window.parent = parent
        window.frame = Frame(window.parent,  background = "white")
        window.parent.title(title)
        window.parent.resizable(width=False, height=False)
        window.frame.pack(expand = True, fill = "both")
        window.centerWindow(window,w,h)# 400, 450)



class interface(Frame, interface_op):
    def __init__(self,parent):
        interface_op.init_window(self,parent,"menu", 400,450)
        #self.parent = parent
        #self.frame = Frame(self.parent,  background = "white")
        #self.parent.title("program")
        #self.frame.pack(expand = True, fill = "both")
        #self.centerWindow(self, 400, 450)
...
class statistics_info(interface_op):
    def __init__(self,parent):
        interface_op.init_window(self,parent,"statistics", 700,400)
        #self.parent = parent
        #self.frame = Frame(self.parent,  background = "white")
        #self.parent.title("statistics")
        #self.frame.pack(expand = True, fill = X)
        #self.centerWindow(self, 700,400)
    ...
    def statistics_menu(self, frame):
        ...
        try:
            ...
            self.fill_table(elements, self.list_box)
        except:
...
class maps_window(interface_op):
    def __init__(self,parent):
        #self.parent = parent
        #self.frame = Frame(self.parent,  background = "white")
        #self.parent.title("maps")
        #self.frame.pack(expand = True, fill = "both")
        #self.centerWindow(self, 600, 350)
        #self.parent.resizable(width=False, height=False)
        interface_op.init_window(self,parent,"map", 600,350)
    ...
    def maps(self,frame):
        ...
        try:
            ...
            self.fill_table(elements, self.list_box_map)
        except:
...
class new_map(interface_op):
    def __init__(self,parent):
        ...
        #self.parent.resizable(width=False, height=False)
        #self.frame.pack(expand = True, fill = "both")
        #self.centerWindow(self, 400, 550)
        interface_op.init_window(self,parent,"new map", 400,550)
    ...
    def compose_data(self, data, parent):
        if self.validation(data, parent):
...
class new_simulation(interface_op):
    def __init__(self,parent, map,b):
        self.b = b
        self.map = map
        self.data_sim =[]
        interface_op.init_window(self,parent,"new simulation", 400,800)
        #self.parent = parent
        #self.parent.resizable(width=False, height=False)
        #self.frame = Frame(self.parent,  background = "white")
        #self.parent.title("new simulation")
        #self.frame.pack(expand = True, fill = "both")
        #self.centerWindow(self, 400, 800)
    ...
    def compose_data(self):
        ...
        if self.validation(data, self.parent, 1):
    ...
    #def open_progress_window(self):
     #   self.new_m_mode = Toplevel(self.parent)
      #  program = progress_page(self.new_m_mode)
...
class fasad:
    ...
    def start_simulation(self):
        ...
        itr = list_iterator(self.sim_data,-1)
        while True:
            try:
                itr.next()
            ...
            param = itr.current()
            id = itr.cursor +1
            self.new_simulation = Simulation.Simulation(id, self.new_statistics, self.map, param[0], param[1], param[2], param[3], param[4], param[5],  param[6])
