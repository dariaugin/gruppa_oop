...
int main()
{
    ...
    std::ifstream flow("C:\\Users\\UserFree\\Desktop\\2_course\\OOP\\Hunters_&_pray\\buffer.txt");
    flow >> width;
    flow >> height;
    flow.close();
    std::ofstream outflow("C:\\Users\\UserFree\\Desktop\\2_course\\OOP\\Hunters_&_pray\\buffer.txt");
    outflow.close();
    ...
    while (window.isOpen())
    {
        ...
        std::ifstream signalFlow("C:\\Users\\UserFree\\Desktop\\2_course\\OOP\\Hunters_&_pray\\signal.txt");
        signalFlow >> command;
        signalFlow.close();
        if (command == "close")
        {
        ...
        /*
        sf::Time finish = clock.getElapsedTime();
        time += finish - start;
        if (time.asSeconds() >= 1)
        {
