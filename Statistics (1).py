...
class Statistics:
    ...
    def BarDiagramPrey(self, time):
        ...
        preyNums = []
        for key in list(keys):
            preyNums.append(self.simulationsData[key][2][time])
        ...
        plt.title("Simulations comparison by prey num on the {} day".format(time))
        plt.xlabel("Simulation num")
        plt.ylabel("Prey num")
        plt.xticks(np.arange(len(keys)), list(keys))    
...
if __name__ == "__main__":
    ...
    DataCollector.BarDiagramHunters(1)
    DataCollector.HuntersPerTime(1)
    DataCollector.HuntAndPreyPerTime(1)
    DataCollector.HuntToPreyPerTime(1)